    ...
    public class AutorizationManagerService : IAutorizationManagerService
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var safeAuth = new SafeAuthModel()
                    {
                        ...
                        AutId = 1,
                        UserId = usr.Id,
                        Control = await _securLib.EncryptLineToStringAsync(usr.Id + "1" + AuthType.Root.ToString()).ConfigureAwait(false)
                    };
        ...
        public async Task AutorizationModify(string p_PeopleModelIdRequest, string p_PeopleModelId, AuthType p_newAuthorization)
        {
            try
            {
                ...
                if (p_newAuthorization < Usr.AutorizationLevel.AuthValue && await _context.Peoples.AnyAsync(p => p.AutorizationLevel.AuthValue == Usr.AutorizationLevel.AuthValue && p.Id != Usr.Id).ConfigureAwait(false) &&
                    await _context.Peoples.AnyAsync(p => p.AutorizationLevel.AuthValue == AuthType.Root && p.Id != Usr.Id).ConfigureAwait(false))
                {
        ...
        private async Task ModifySafeAut(string p_UserId, int p_newAuthorization, AuthType p_NewAuthType)
        {
            try
            {
                ...
                authSaved.Control = await _securLib.EncryptLineToStringAsync(p_UserId.ToString() + p_newAuthorization.ToString() + p_NewAuthType.ToString()).ConfigureAwait(false);
